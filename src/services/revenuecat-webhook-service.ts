...
import type { RevenueCatWebhookPayload, RevenueCatEventType } from './dtos/revenuecat';
...
export class RevenueCatWebhookService {
  ...
  async processWebhook(payload: RevenueCatWebhookPayload): Promise<ServiceResult<void>> {
    try {
      ...
      // Allow sandbox events in non-production environments for testing
      if (event.environment === 'SANDBOX') {
        this.logger.info('Processing sandbox event', { eventId: event.id });
      }
      ...
      if (!lockIdAttr?.value) {
        ...
        return failure(
          'MISSING_LOCK_ID',
          'lock_id attribute missing from purchase',
          undefined,
          400
        );
      }
      ...
      if (!Number.isFinite(lockId) || lockId <= 0) {
        this.logger.error('Invalid lock_id in subscriber attributes', {
          lockIdValue: lockIdAttr.value,
        });
        return failure(
          'INVALID_LOCK_ID',
          'Invalid lock_id in subscriber attributes',
          undefined,
          400
        );
      }
  ...
  private async handleUnsealPurchase(
    lockId: number,
    eventId: string
  ...
  private async handleStorageUpgradePurchase(
    lockId: number,
    eventId: string
  ): Promise<ServiceResult<void>> {
    try {
