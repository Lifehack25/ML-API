...
): ServiceContainer => getContainer(c);
...
export const createUserRoutes = (config: AppConfig) => {
  ...
  router.post('/verify/send-code', validateJson(sendCodeSchema), async (c) => {
  ...
  router.post('/verify/resend-code', validateJson(resendCodeSchema), async (c) => {
    const payload = c.req.valid('json') as { isEmail: boolean; identifier: string };
    const result = await getService(c).services.users.resendTwilioCode(
      payload.isEmail,
      payload.identifier
    );
    if (result.ok) {
  ...
  router.post('/auth/refresh', validateJson(refreshSchema), async (c) => {
  ...
  router.patch('/me/name', validateJson(updateNameSchema), async (c) => {
    const userId = getUserId(c);
  ...
  router.put('/me/device-token', validateJson(deviceTokenSchema), async (c) => {
    const userId = getUserId(c);
