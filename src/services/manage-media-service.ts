...
import type { CloudflareMediaClient } from '../infrastructure/cloudflare-media';
...
/**
 * Service for handling media operations on locks/albums.
 * Orchestrates validations, content moderation, Cloudflare uploads, and database updates.
 */
export class ManageMediaService {
  ...
  private async validateStorage(
    ...
    try {
      ...
      const maxImages = upgraded
        ? this.storageLimits.tier2ImageLimit
        : this.storageLimits.tier1ImageLimit;
      const maxVideoSeconds = upgraded
        ? this.storageLimits.tier2VideoSeconds
        : this.storageLimits.tier1VideoSeconds;

      const existingMedia = data.media.filter((m) => !m.isMainImage);
      const imageCount = existingMedia.filter((m) => m.isImage).length;
      const videoSeconds = existingMedia
        .filter((m) => !m.isImage)
  ...
  /**
   * Uploads a single media file to an album.
   * Performs:
   * 1. Storage limit validation (Tier 1 vs Tier 2)
   * 2. Content moderation (Sightengine)
   * 3. Upload to Cloudflare (Images or Stream)
  ...
  async uploadSingleMedia(payload: UploadMediaPayload): Promise<ServiceResult<CreatedMedia>> {
    ...
    if (!moderation.approved) {
      return failure(
        'MODERATION_REJECTED',
        moderation.rejectionReason ?? 'Content rejected',
        undefined,
        400
      );
    }
    ...
    if (
      ...
      this.logger.warn(
        'Cloudflare rejected original image (too large). Retrying with compressed version.',
        {
          originalSize: file.size,
          compressedSize: moderation.compressedImage.size,
  ...
  /**
   * Publishes batch metadata changes for an album.
   * Handles deletions, reordering, and main image updates in a transactional manner.
   * Schedules async cleanup for deleted media on Cloudflare.
   */
  async publishMetadata(request: PublishMetadataRequest): Promise<ServiceResult<PublishResult>> {
    ...
    const mainImageUpdates = changes.filter(
      (c) => c.changeType === MetadataChangeType.UpdateMainImage
    );
    ...
    try {
      ...
      for (const change of deletes) {
        if (change.mediaId) {
          batchQueries.push(
            this.db.delete(mediaObjects).where(eq(mediaObjects.id, change.mediaId))
          );
        }
      }
      ...
      for (const change of reorders) {
        if (
          change.mediaId &&
          change.newDisplayOrder !== undefined &&
          change.newDisplayOrder !== null
        ) {
          batchQueries.push(
            this.db
              .update(mediaObjects)
      ...
      if (albumTitle) {
        batchQueries.push(
          this.db.update(locks).set({ album_title: albumTitle }).where(eq(locks.id, lockId))
        );
      }
      ...
      for (const media of mediaToCleanup) {
        try {
          ...
          this.logger.info('Scheduled Cloudflare cleanup job', {
      ...
      return failure(
        'METADATA_PUBLISH_FAILED',
        `Failed to publish album: ${String(error)}`,
        undefined,
        500
      );
    }
